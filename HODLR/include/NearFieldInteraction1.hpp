struct integrandInputs {
  pts2D X;
  int p;//summation term
  int l;//basis index
  pts2D center_B;//center of box B
  pts2D center_A;//center of box A
  int level_A;
  int level_B;
  double radius_B;
  double radius_A;
...
const double GAMMA = 0.57721566490153286060;//Euler's constant

class NearFieldInteraction1: public Quadrature {
  /*
...
public:
  ...
  NearFieldInteraction1(int nChebNodes, integrandInputs II, int degreeOfBases): Quadrature() {
    this->nChebNodes = nChebNodes;
    this->rank = nChebNodes*nChebNodes;
    this->II = II;
    nNodes = 10;//quadrature class object
    this->epsilon = 1e-7;
    this->NLEVELS = 10;
  ...
  double factorial(int n) {
    return (n==1 || n==0) ? 1.0: double(n) * factorial(n - 1);
  }
  ...
  double mybesselJ(int n, double x) {
    double sum = 0.0;
    for (size_t p = 0; p <= pMax; p++) {
      sum += ap(p,x)/pow(2.0, 2.0*p);
    }
    return sum;
  }

  double mybesselY(int n, double x) {
    double sum = (log(x/2)+GAMMA)*besselJ(n,x);
    for (size_t p = 1; p < pMax; p++) {
      sum += -Hp(p)*ap(p,x)/pow(2.0,2.0*p);
    }
    return 2.0/PI*sum;
  }

  //the following two functions evaluateV and integrand use no series expansions

  void evaluateV(Mat &V1) {
    V1 = Mat::Zero(rank, NumberOfBases);
    std::vector<pts2D> Nodes_A;
    shift_scale_Nodes(standardChebNodes, Nodes_A, II.center_A.x, II.center_A.y, II.radius_A);
    for (size_t i = 0; i < rank; i++) {
      for (size_t l = 0; l < NumberOfBases; l++) {
        ...
        II.l = l;
        xcenter = 0.0; //of Quadrature object
        ycenter = 0.0; //of Quadrature object
        Lx = 1.0; //of Quadrature object
        Ly = 1.0; //of Quadrature object
  ...
  dtype integrand(pts2D P) {
    ...
    int a = op.x;
    int b = op.y;
    double arg = kappa*II.radius_B*R;
    //cout << "arg: " << arg << " II.radius_B: " << II.radius_B << endl;
    return (besselJ(0,arg)+I*besselY(0,arg))*get_ChebPoly(P.x, a)*get_ChebPoly(P.y, b);
	};


  //the following two functions evaluateV1 and integrand use series expansions
  /*
  void evaluateV1(Mat &V1) {
    ...
    for (size_t i = 0; i < rank; i++) {
      for (size_t l = 0; l < NumberOfBases; l++) {
        for (size_t p = 0; p <= pMax; p++) {
          ...
          obtain_Quadrature_Uniform();//when an integrand is evaluated gauss Quadrature tree is constructed fresh
          //cout << "i: " << i << " l: " << l << "  p: " << p << "  " << cp(p, kappa*II.radius_B) << ", " << integral << endl;
          V1(i,l) += cp(p, kappa*II.radius_B) * integral;
        }
      }
    }
  }
  ...
  dtype integrand(pts2D P) {
    ...
    int a = op.x;
    int b = op.y;
    return pow(R/2.0, 2.0*II.p)*get_ChebPoly(P.x, a)*get_ChebPoly(P.y, b);
	};
  */
