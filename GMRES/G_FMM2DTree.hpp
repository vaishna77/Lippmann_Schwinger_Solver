...
// public:
	...
	kernel_dtype getMatrixEntry(const unsigned i, const unsigned j) {
		...
		double R = sqrt(R2);
		// kernel_dtype out = exp(I*kappa*R)/R;
		// kernel_dtype out = 1/R;
		// kernel_dtype out = 1/(1+R*R);
		// kernel_dtype out = log(R);
		...
		if (tree[j][k].active == true) {
			...
			if (n_rows != 0 && n_cols != 0) {
				...
				if(ComputedRank > 0) {
					...
					//////////////////// checking ranks of ACA and NCA /////////////////
					// if (j==nLevels-1) {
					// 	std::vector<int> row_bases1, col_bases1;
					// 	double start, end;
					// 	start		=	omp_get_wtime();
					// 	Mat dummy2;
					// 	int nestedRank;
					// 	ACA_only_nodes(row_bases1, col_bases1, nestedRank, tol_ACA, dummy, dummy2);
					// 	end		=	omp_get_wtime();
					// 	double timeNested =	(end-start);
					// 	std::vector<int> row_indices_local1;
					// 	for (size_t r = 0; r < row_bases1.size(); r++) {
					// 		row_indices_local1.push_back(IL_Nodes[row_bases1[r]]);
					// 	}
					// 	std::vector<int> col_indices_local1;
					// 	for (size_t c = 0; c < col_bases1.size(); c++) {
					// 		col_indices_local1.push_back(boxA_Nodes[col_bases1[c]]);
					// 	}
					//
					...
					// 	cout << "nested rank: " << nestedRank << "	nested error: " << nested_error << "	timeNested: " << timeNested << endl;
					//
					// 	// non nested
					// 	row_indices = IL_charges;
					// 	col_indices = tree[j][k].chargeLocations;//object of base class G_LowRank
					// 	std::vector<int> row_bases_2, col_bases_2;
					// 	start		=	omp_get_wtime();
					//   ACA_only_nodes(row_bases_2, col_bases_2, ComputedRank, tol_ACA, dummy, dummy2);
					// 	end		=	omp_get_wtime();
					// 	double timeNonNested =	(end-start);
					// 	cout << "ACA done" << endl;
					// 	std::vector<int> row_indices_local2;
					// 	for (size_t r = 0; r < row_bases_2.size(); r++) {
					// 		row_indices_local2.push_back(IL_charges[row_bases_2[r]]);
					// 	}
					// 	std::vector<int> col_indices_local2;
					// 	for (size_t c = 0; c < col_bases_2.size(); c++) {
					// 		col_indices_local2.push_back(tree[j][k].chargeLocations[col_bases_2[c]]);
					// 	}
					// 	Atilde = getMatrix(row_indices_local2, col_indices_local2);
					// 	cout << "Atilde done" << endl;
					// 	ColPivHouseholderQR<Mat> U = Atilde.colPivHouseholderQr();
					// 	C = getMatrix(IL_charges, col_indices_local2);
					// 	R = getMatrix(row_indices_local2, tree[j][k].chargeLocations);
					// 	A_cur = C*U.solve(R);
					// 	cout << "A_cur done" << endl;
					// 	double non_nested_error = (Afull-A_cur).norm()/Afull.norm();
					// 	cout << "non nested rank: " << ComputedRank << "	non nested error: " << non_nested_error << "	timeNonNested: " << timeNonNested << endl;
					// 	cout << "non nested size: " << row_indices.size() << ", " << col_indices.size() << endl;
					// 	exit(0);
					// }
					////////////////////////////////////////////////////////////////////
				}
