...
HODLR_Node::HODLR_Node(int level_number, int node_number, int local_number,
                       int n_start, int n_size, double tolerance, int target_rank
                      )
{
...
void HODLR_Node::assembleNonLeafNode(LowRank* F, bool is_sym)
{
    if(is_sym == true)
    {
        F->getFactorization(U[0], V[1], tolerance, target_rank, c_start[0], c_start[1], c_size[0], c_size[1]);
        V[0]    = U[0];
        U[1]    = V[1];
        rank[0] = U[0].cols();
        rank[1] = rank[0];
    ...
    else
    {
        F->getFactorization(U[0], V[1], tolerance, target_rank, c_start[0], c_start[1], c_size[0], c_size[1]);
        rank[0] = U[0].cols();
        F->getFactorization(U[1], V[0], tolerance, target_rank, c_start[1], c_start[0], c_size[1], c_size[0]);
        rank[1] = U[1].cols();
    }
}
